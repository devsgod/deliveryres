...
export class HomePage implements OnInit {
  ...
  constructor(
    ...
    this.cusineSeleted = ["Mediterranean", "Middle Eastern", "Vegan", "Salad Bar", "Halal & Kosher", "Italian", "Mexican", "Chinese", "Indian", "Japanese"];
  }
      ...
      if (data && data.length) {
        ...
        data.forEach(async (element) => {
          if (element && element.isClose === false && element.city === this.cityId) {
            ...
            if (element.cusineDP) {
              if (this.delipick){
                if (element.cusineDP[0] == "Delivery"){
                    const intersection = this.cusineSeleted.filter(item => element.cusine.includes(item));
                ...
                if((element.cusineDP[0] == "Delivery" && element.cusineDP[1] == "Pickup") || element.cusineDP[0] == "Pickup" ){
                    const intersection = this.cusineSeleted.filter(item => element.cusine.includes(item));
                    console.log(intersection);
                    if (intersection.length>0){
                    element.time = moment(element.time).format('HH');
  ...
  cusineSelete(){
    console.log(this.cusineSeleted);
    this.getRest();
  }
}
